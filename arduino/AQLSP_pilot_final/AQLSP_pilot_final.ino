...
char program[18] = "AQLSP_pilot_final"; // record program version in data.file
...
#endif
int LEDPIN = 6; //data pin plugged in to digital pin 6
int NUMPIXELS = 8;
...
File dataFile;
String sdText;
int ledsd = 7; //neopixel led associated with mod
...
int ledtemp = 6; //neopixel led associated with mod
int tempranges[] = {40, 50, 60, 70, 80, 90}; //ranges for color categories
...
String co2Text;
int k30led = 5; //neopixel led associated with mod
int k30ranges[] = {500, 600, 700, 850, 1100, 1500}; //ranges for color categories
...
String ozoneText;
int ozoneled = 4; //neopixel led associated with mod
int ozoneranges[] = {54, 70, 85, 105, 200, 100}; //ranges for color categories
...
int ada2_25;
int ada2_100;
int ledada1 = 0; //neopixel led associated with mod 1
int ledada2 = 1; //neopixel led associated with mod 2
int pm25ranges[] = {12, 35.4, 55.4, 150.4, 250.4, 500};
int led1pm10 = 2; //neopixel led associated with mod 1
int led2pm10 = 3; //neopixel led associated with mod 2
int pm10ranges[] = {54, 70, 85, 105, 200, 300}; //ranges for color categories
...
void setup() {
  ...
  delay(5000);
  lcdscreenPrint(ada2_25Text, ozoneText, bmeText, String("you look nice today!"));
  delay(5000);
}
...
void loop() {
  ...
  if (now.minute() % 59 == 0) {
    ...
    // Enter light sleep mode between data collection cycles.
    enterLightSleepMode();
  }
}
...
void adjustLCDBacklight(int hour) {
  if (hour >= 20 || hour < 8) {
    ...
    lcd.noBacklight(); // Turn off backlight completely at night
  } else {
